...
function checkName() {
  ...
  if (theName === "" || theName.length < 2) {
    ...
    document.getElementById("nameFeedback").innerHTML =
      "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>不可為空白，至少兩字以上";
    flag1 = false;
  } else {
    flag1 = true;
    let reg = /^[\u4e00-\u9fff]*$/;
    if (reg.test(theName)) {
      ...
      flag2 = false;
      document.getElementById("nameFeedback").innerHTML =
        "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>請輸入中文";
    }
  }
  if (flag1 && flag2) {
    document.getElementById("nameFeedback").innerHTML =
      "<img src='images/correct.gif' style='width: 20px; margin:0px 5px -3px 0;'>正確";
    correctName = true;
  }
}
...
function checkPwd() {
  ...
  if (thePwd === "") {
    document.getElementById("pwdFeedback").innerHTML =
      "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>不可為空白";
  }
  //至少6字
    ...
    if (flag1 && flag2 && regSpe.test(thePwd)) {
      document.getElementById("pwdFeedback").innerHTML =
        "<img src='images/correct.gif' style='width: 20px; 18px; margin:0px 5px -3px 0;'>正確";
      correctPwd = true;
    } else if (!flag1) {
      document.getElementById("pwdFeedback").innerHTML =
        "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>請輸入英文";
    } else if (!flag2) {
      document.getElementById("pwdFeedback").innerHTML =
        "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>請輸入數字";
    } else if (!regSpe.test(thePwd)) {
      document.getElementById("pwdFeedback").innerHTML =
        "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>必須包含特殊字元";
    }
  } else {
    document.getElementById("pwdFeedback").innerHTML =
      "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>至少6個字以上";
  }
...
function checkDate2() {
  ...
  if (arr.length == 3 && arr[0] * 1 > 0 && arr[1] * 1 > 0 && arr[2] * 1 > 0) {
    ...
    if (myInputDay * 1 > lastday * 1) {
      document.getElementById("dateFeedback").innerHTML =
        "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>此日期不存在^q^";
    } else if (
      myInputDay * 1 <= lastday * 1 &&
      arr.length == 3 &&
      arr[1] != "" &&
      arr[2] != ""
    ) {
      document.getElementById("dateFeedback").innerHTML =
        "<img src='images/correct.gif' style='width: 20px; 18px; margin:0px 5px -3px 0;'>";
      correctDate2 = true;
    }
  }
  //驗證只能輸入數字.字數
  ...
  if (
    ...
    document.getElementById("dateFeedback").innerHTML =
      "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>請輸入完整日期";
  }
  if (theIpDate === "") {
    document.getElementById("dateFeedback").innerHTML =
      "<img src='images/error.png' style='width: 18px; margin: 0 5px -1px 0;'>空白ㄉ查不到呐~";
  }
}
